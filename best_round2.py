...
class BasketArbitrageStrategy(MarketMakingStrategy):
    ...
    def act(self, state: TradingState):
        ...
        # Fair value spreads and z-scores already computed above
        position = pos.get(self.symbol, 0)

        if self.symbol == "PICNIC_BASKET1":
            size = self.scaled_trade_size(z1, self.limit)
            if z1 > 1.0 and position > -self.limit:
                self.sell(pb1, min(size, self.limit + position))
            elif z1 < -1.0 and position < self.limit:
                self.buy(pb1, min(size, self.limit - position))

        elif self.symbol == "PICNIC_BASKET2":
            size = self.scaled_trade_size(z2, self.limit)
            if z2 > 1.0 and position > -self.limit:
                self.sell(pb2, min(size, self.limit + position))
            elif z2 < -1.0 and position < self.limit:
                self.buy(pb2, min(size, self.limit - position))

                # Trade components based on basket spreads
    ...
    def load(self, *args):
        if self.symbol == "CROISSANTS" and args:
            self.croissant_data = deque(args[0], maxlen=100)
        elif self.symbol == "JAMS" and args:
            self.jam_data = deque(args[0], maxlen=100)
        elif self.symbol == "DJEMBES" and args:
            self.djembe_data = deque(args[0], maxlen=100)
    ...
    def save(self) -> JSON:
        if self.symbol == "CROISSANTS":
            ...
            return None  # important for basket symbols!


    def scaled_trade_size(self, z: float, limit: int, max_cap: int = 10) -> int:
        return int(min(max_cap, max(1, abs(z * limit / 5))))
...
class Trader:
    def __init__(self) -> None:
        ...
        self.strategies = {symbol: clazz(symbol, limits[symbol]) for symbol, clazz in {
            ...
            "PICNIC_BASKET1": BasketArbitrageStrategy,
            "PICNIC_BASKET2": BasketArbitrageStrategy,
